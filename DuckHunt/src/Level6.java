...
public class Level6 extends Application {
    ...
    private DuckPane duckSecond;
    private DuckPane duckThird;
    private Pane ducks = new Pane();
    ...
    private String winSoundPath = "assets/effects/GameCompleted.mp3";
    ...
    /**
     * Constructor for the Level 6
     * This level includes 3 ducks flying diagonally and 9 bullets.
     ...
     */
    public Level6(Image crosshairSelected, Image backgroundSelected,Image foregroundSelected) throws URISyntaxException {
        ...
        duckSecond = new DuckPane(backgroundSelected,"blue");
        duckThird = new DuckPane(backgroundSelected,"red");
        duckSecond.setDuckX(backgroundSelected.getWidth()* DuckHunt.SCALE - duckSecond.getDuckWidth());
        duckSecond.setDuckY(duckSecond.getDuckHeight()*2);
        duckThird.setDuckX(0);
        duckThird.setDuckY(backgroundSelected.getWidth()* DuckHunt.SCALE - duckSecond.getDuckHeight());
        ...
        winText.setTranslateY((backgroundSelected.getHeight() * DuckHunt.SCALE / 2) - winText.getLayoutBounds().getHeight() / 2 -20* DuckHunt.SCALE);
    ...
    /**
     * This method is called to start the Level6 scene.
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        ducks.getChildren().addAll(duckFirst.getDuckPane(),duckSecond.getDuckPane(),duckThird.getDuckPane());
        root.getChildren().addAll(ducks);
        root.getChildren().add(foregroundPane);


        levelScene = new Scene(root,backgroundSelected.getWidth()* DuckHunt.SCALE,backgroundSelected.getHeight()* DuckHunt.SCALE);
        duckFirst.playDiagonal();
        duckSecond.playDiagonal();
        duckThird.playDiagonal();
        ...
        levelScene.setOnMouseClicked(e->{
            ...
            if(ammo>0 && duckCount==0){
                ...
                //get the mouse coordinates and hit the duck if it is in the area
                ...
                if (mouseX >= duckSecond.getDuckX() && mouseX <= duckSecond.getDuckX() + duckSecond.getDuckWidth() && mouseY >= duckSecond.getDuckY() && mouseY <= duckSecond.getDuckY() + duckSecond.getDuckHeight() && !duckSecond.isDuckHit()) {
                    duckSecond.stopAnimation();
                    duckSecond.hitDuck();
                    duckSecond.playFall();
                    duckCount--;
                }

                if (mouseX >= duckThird.getDuckX() && mouseX <= duckThird.getDuckX() + duckThird.getDuckWidth() && mouseY >= duckThird.getDuckY() && mouseY <= duckThird.getDuckY() + duckThird.getDuckHeight() && !duckThird.isDuckHit()) {
                    duckThird.stopAnimation();
                    duckThird.hitDuck();
                    duckThird.playFall();
                    duckCount--;
                }
     ...
     * It handles the key events of the game such as the player pressing the enter key to start again from the first level.
     ...
     * It handles the key events of the game such as the player pressing the enter key to restart from the first level.
